...
## Brief
Working for the first time as a group of 3, myself, [Tom](https://github.com/TMurp) and [Marilyn](https://github.com/marilynpoku) over a period of 8 days created a full stack front-end website and back-end database for our 3rd project during the Software Engineering Immersive (SEI) bootcamp at General Assembly.      
...
On the home page, you can discover new locations from randomly generated carousels and some of the highest rated places too. Here, you can also search by name and type of place that suits your interests.   
...
We then created a trello Kanban workspace and roughly planned out the theme, design, user stories, components and file structure. We each would then take a component and start building the front-end. The back-end was done together at once with myself leading the coding while the others screen watched and helped. We assigned components based on a couple things, one being preference on what we wanted to practice and which components would not be reliant on another that was being made by one of us. This way we would all have something to work on at the same time throughout most of the build.     
...
#### Nav-Bar
Nav bar contains links to home, map and profile pages. The search bar is contained on the home page so the search link will take you to the home page and using react-router-hash-link, scroll the page down to the search. While the user is not logged in, buttons to login & register are displayed.     
...
After logging in, the button switches to logout and a link to the users profile page. The site name and diamond logo (Nav.Brand) in the top left will link you to the home page.      
...
In mobile view, the navbar collapses into a dropdown activated by clicking on the "hamburger" icon.     
...
The showcase shows the most information about the Gem record. Here users will be able to learn all the details the creator has added about the Place. At the bottom is a mini version of the map that just shows the location of the place. If users are logged in, they will be able to have further interactions with the page such as giving a rating to the Gem. Hovering over the gem rating icon will cause it to pulse and on click/tap the rating is sent through a put request and added. The average rating is updated, that score is updated in real time and the number of diamonds changes is the score dictates it to. Logged in users will also be able to type and leave comments. If they are the comment owner, they can either edit or delete it, they will be able to like other comments too. If they click like on a comment they have liked before, the like will be removed and the count will display one less like total.   
...
## Challenges
- This was my first time working with a map and React-Mapbox-gl was a challenge to get the components to function as I wanted, I needed to read documentation to better understand where my issues were.
- This was my first time creating a search system with more than 2 variables, in the case of this project it was to search by a category and a tag along with text. In the end this was simplified to category and tag until we go back and make improvements.
...
- First time creating a back-end from the ground up, we worked closely together, had some trouble with populating our models with nested schema information but it came out very well in the end.
...
## Main Takeaways
- It took some time to get it to the point where it is now, but I really liked build a map with mapbox-gl, I loved adding more features to it and will continue to research how to improve it. 
- Working collaboratively as a group, using Github and planning out each section of the site was a challenge at first but the more we got into the habit the easier it became. 
- I saw myself being more confident in reading package documentation to implement features not as familiar to myself.
- I gained more React framework fluency and is now becoming one of my favourite systems to work with.
